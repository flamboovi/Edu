...
module.exports.useCache = async (req, res, next) => {
   try {
   const cache = 
   getCachedAPIData(req.originalUrl);
   if (cache) {
...
const getMessage = (handler, locale, code) => {
  return messageCode[handler][locale] 
 [code];
};
 ...
 const successResponse = (
    ...
    let responseData = {
status: " success",
message: getMessage(handler, req.handlers.locale || "en", messageCode),
messageCode,
success,
data,
 };
...
if (cache) 
  setCachedAPIData(req.originalUrl,responseData);
 return responseData;
};
